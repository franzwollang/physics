...
### 4.3 Classification (Proximity Clustering)

In a pure U(1) scalar theory, stable topological junctions are forbidden. However, baryonic matter in this framework is hypothesized to be "tangled/knotted" defect complexes. Therefore, we classify structures based on spatial proximity:

1.  **Extract Loops:** Identify all connected components of the dual vortex graph.
2.  **Cluster:** Compute the minimum distance $d_{ij}$ between every pair of loops. Cluster loops together if $d_{ij} < R_{\text{prox}}$, where $R_{\text{prox}} \approx 2\ell$ (twice the core width).
3.  **Classify:**
    -   **Isolated Loop (Cluster Size = 1):** Represents **Dark Matter** candidate.
    -   **Clustered Group (Cluster Size > 1):** Represents **Baryonic Matter** (complex, interacting tangle).
...
```json
{
  ...
  "outputs": {
    ...
    "num_isolated_loops": 30,
    "num_clustered_groups": 4,
    "loop_lengths": [12, 24, 8, ...],
    "total_vortex_length": 350,
    "length_isolated": 250,
    "length_clustered": 100,
    "ratio_dm_baryon": 2.5,
...
|-------|---------|-------------------------|
| `num_isolated_loops` | Count of spatially isolated loops | Proxy for number of DM candidates |
| `num_clustered_groups` | Count of tangled loop clusters | Proxy for baryon-like structures |
| `ratio_dm_baryon` | `length_isolated / length_clustered` | DM-to-Baryon mass ratio estimate |
...
### 9.5 Missing Junctions (U(1) Limitation)

-   **Issue:** Standard U(1) scalar theory does not support stable topological Y-junctions; defects are always loops. True baryonic junctions require non-Abelian or $Z_N$ symmetry breaking.
-   **Mitigation:** We use **Proximity Clustering** as an effective proxy. We assume that in the full theory, regions where multiple loops are "tangled" or effectively overlapping would condense into stable junctions. Thus, "clustered loops" are the baryonic proxy.
