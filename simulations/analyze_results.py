...
def analyze_results(results_dir: Path) -> None:
    ...
    print(f"{'Lambda':<8} | {'Beta':<6} | {'Ell':<6} | {'Runs':<4} | {'Ratio(I/C)':<12} | {'Iso Length':<12} | {'Clus Length':<12} | {'# Iso':<8} | {'# Clus':<8}")
    print("-" * 110)
    ...
    for lam in sorted(by_lambda_full.keys()):
        ...
        for d in runs:
            out = d['outputs']
            r = out.get('ratio_dm_baryon')
            if r is not None:
                ratios.append(r)
            else:
                # Handle effectively infinite ratio (all DM)
                if out.get('length_clustered', 0) == 0 and out.get('length_isolated', 0) > 0:
                    ratios.append(float('inf'))
            
            iso_lens.append(out.get('length_isolated', 0))
            clus_lens.append(out.get('length_clustered', 0) + out.get('length_junctions', 0))
            num_isos.append(out.get('num_isolated_loops', 0))
            num_clus.append(out.get('num_clustered_groups', 0) + out.get('num_junction_clusters', 0))

        # Filter infs for mean calculation
        finite_ratios = [x for x in ratios if x != float('inf')]
        avg_ratio = statistics.mean(finite_ratios) if finite_ratios else 0.0
        n_inf = len(ratios) - len(finite_ratios)
        
        ratio_str = f"{avg_ratio:.3f}"
        if n_inf > 0:
            ratio_str += f" (+{n_inf} Inf)"
        ...
        print(f"{lam:<8.3f} | {beta:<6.3f} | {ell:<6.2f} | {n:<4} | {ratio_str:<12} | {avg_iso:<12.1f} | {avg_clus:<12.1f} | {avg_n_iso:<8.1f} | {avg_n_clus:<8.1f}")
