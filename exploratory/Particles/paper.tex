...
\subsection{Existence Proof: Bound-State Count}
We can formalize the "Why Three?" argument by considering the radial texture equation for a defect in a spherical square-well stiffness profile: $w^2(r)=w_c^2$ for $r<R$ (core) and $w^2(r)=w_v^2\ll w_c^2$ for $r>R$. Write $E=-\varepsilon<0$ for bound states and define $k=\sqrt{\varepsilon/(\kappa w_c^2)}$, $\alpha=\sqrt{\varepsilon/(\kappa w_v^2)}$. Inside the core the equation reduces to
\begin{equation}
  \psi'' + \frac{2}{r}\psi' + \Big(k^2 - \frac{\ell(\ell+1)}{r^2}\Big)\psi = 0,
\end{equation}
whose regular solutions are $j_\ell(kr)$. Outside, with $w_v^2\ll w_c^2$, decay solutions are modified spherical Bessel $k_\ell(\alpha r)\sim e^{-\alpha r}$. Matching $\psi$ and $\psi'$ at $r=R$ yields the quantization condition.
For a deep, wide core ($\alpha R\gg 1$), the first bound state for each angular momentum $\ell$ enters when $kR > (\ell+1/2)\pi$. Choosing parameters such that
\begin{equation}
  \frac{5\pi}{2} < kR < \frac{7\pi}{2}\qquad\text{and}\qquad \alpha R \gg 1
\end{equation}
guarantees exactly three bound levels: $(\ell=0,1,2)$ lie below the continuum, while the $\ell=3$ mode is unbound. This interval is wide ($\Delta kR=\pi$), so the result is robust. Mapping back to the framework, $\ell$ labels the geometric grade of the internal rotor texture; the existence of precisely three sub-gap modes realizes the three-generation hypothesis without fine tuning.
...
\subsection{Quantitative Scaling Estimate}
Using the stiffness hierarchy derived in the foundational Infinite-Clique Graph papers ($\kappa \ll \beta_{\rm curv} \ll B_{\rm vol}$), we can estimate the grade energies:
\begin{equation}
  m_1 \;\propto\; \kappa,\qquad
  m_2 \;\propto\; \kappa + \beta_{\rm curv},\qquad
  m_3 \;\propto\; \kappa + \beta_{\rm curv} + B_{\rm vol}.
\end{equation}
With the bounds $\beta_{\rm curv}/\kappa\simeq 200$ and $B_{\rm vol}/\beta_{\rm curv}\simeq 16$ (derived from the graph rigidity transition), the ratios become:
\begin{equation}
  m_1 : m_2 : m_3 \;\approx\; 1 : 200 : 3500.
\end{equation}
This matches the observed charged lepton hierarchy ($m_e \approx 0.5$ MeV, $m_\mu \approx 105$ MeV, $m_\tau \approx 1776$ MeV) remarkably well ($1 : 207 : 3500$) up to order-one geometric factors. Thus, the mass hierarchy reflects the spectral hierarchy of the vacuum itself.
...
\end{equation}
with $\mathcal O_{\rm tex}$ defined in Appendix~E and boundary conditions set by the core profile. The three lowest, sub-gap modes ($\lambda_i$ below the spectral gap) span the slow texture manifold and define the generation basis. Stability requires positive second variation of the full free energy along these modes; higher eigenmodes either lie above the gap or open unstable directions and are excluded.

\subsection*{Overlap Suppression Example}
To illustrate the suppression of off-diagonal couplings, consider a 1D toy model with Laguerre polynomial textures $\Xi_n(x) \propto L_n(x)e^{-x/2}$ in a metric background $w_*^2(x)=e^{-x}(1-\eta x)$. The overlap integral is:
\begin{equation}
  \int_0^\infty \Xi_n^\*\Xi_m\,w_*^2 dx \;=\; \delta_{nm} - \eta\big[(2n+1)\delta_{nm} - n\,\delta_{n,m+1} - (n+1)\delta_{n+1,m}\big].
\end{equation}
For a deformation $\eta\simeq 0.15$, the diagonal entries scale as $(0.85, 0.55, 0.25)$ while off-diagonals remain small $\mathcal O(0.1)$. This demonstrates how geometric complexity naturally suppresses the effective couplings for higher generations.
...
\end{equation}
with $\delta_f$ capturing the weak, sector-dependent running. Because the overlap integrals involve normalized $\Xi_i$, the leading running cancels in ratios and in mixing angles, leaving only small, potentially measurable drifts (Section~9).
...
Form $M_f\approx c_f\,S + \delta_f\,\Delta$ with small perturbations $\Delta$ from subleading terms. A representative choice yields
\begin{align}
...
\end{equation}
These numbers realize a light-first-generation hierarchy, small quark mixing, and large atmospheric-type leptonic mixing.

\paragraph{Deliverables (to be included).} Sensitivity analysis for $(c_f)$.
