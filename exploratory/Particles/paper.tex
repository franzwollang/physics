...
While geometric grades explain the generational ladder ($e,\mu,\tau$), they do not immediately explain the fundamental division between leptons (integer charge, unconfined) and quarks (fractional charge, confined). We propose that this distinction arises from a \textbf{Global Chiral Bias} in the vacuum geometry.

\subsection{The Chiral Vacuum: a pseudoscalar $\theta$-condensate (Lorentz-safe)}
We formalize the ``threaded vacuum'' as a \emph{Lorentz-invariant} chiral order parameter: a pseudoscalar field $\theta(x)$ (constant within our observable patch to leading order) that enters the effective rotor-sector action through a CP-odd topological density. For an internal rotor connection with curvature $F_{\mu\nu}$ (SU(2)/SU(3) in the coarse-grained theory), the standard invariant is
\begin{equation}
...
\subsection{Beta Decay: Topology Reconfiguration}
Consider Beta Decay ($n \to p^+ + e^- + \bar{\nu}_e$).
\begin{enumerate}
    \item \textbf{Initial State:} A Neutron ($n$). A bound state of 3 Volume Defects (quarks) with net zero phase winding. The Down Quark ($d$) is a volume defect with effective fractional winding $-1/3$.
    \item \textbf{Trigger:} A topological fluctuation ("pinch") creates a dipole pair of phase windings $(+1, -1)$ from the vacuum. Net charge change is zero.
    \item \textbf{Reconfiguration:}
    \begin{itemize}
        \item The \textbf{$+1$ Winding} merges with the Down quark ($-1/3 + 1 = +2/3$). This transforms the Down quark into an Up quark ($u$), converting the Neutron into a Proton.
        \item The \textbf{$-1$ Winding} is ejected. This is a heavy, unstable Phase Anchor ($W^-$ boson).
    \end{itemize}
    \item \textbf{Decay:} The unstable $W^-$ Anchor decays exactly as in the lepton case: relaxing into the ground state Anchor (Electron) and shedding the transition geometry as an antineutrino ($\bar{\nu}_e$).
\end{enumerate}
