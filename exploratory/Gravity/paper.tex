...
\end{equation}
with $C_{\rm drag}$ an overall coefficient (carrying the required units in local\,unit conventions). The physical picture is aligned with the rest of the framework: forced acceleration shifts the comoving window (an Unruh/KMS effect of the phase cone) and thereby increases the effective bath intensity sampled by the soliton's internal mode. In the comoving (Rindler) description this bath has a small Tolman gradient across any finite-size soliton, with the trailing side (toward the horizon) slightly ``hotter'' than the leading side. This creates an effective $\nabla\tau_{\rm eff}$ across the soliton and therefore a thermodynamic force pointing backward (toward higher noise), i.e. a drag that does negative work and bleeds energy into phase-sector excitations (SI Section~\ref{si:drag}). In all cases, Lorentzian kinematics already enforce $v<c_s$; the drag acts only for non-geodesic motion and further suppresses approaches to $c_s$ under sustained proper acceleration. Massless modes ($m=0$) decouple from this mechanism and remain exactly at $c_s$.
...
\paragraph{Acceleration-induced drag with ultra-relativistic suppression.} The non-geodesic drag law $P_{\rm rad}\propto \gamma_v^4 a_s^2$ (with $P_{\rm work}\approx P_{\rm rad}$ for sustained drive) predicts ultra-relativistic suppression $a_s(v)\propto\gamma_v^{-1}$. We parameterise the residual as a fraction of synchrotron losses and bound it empirically at $\lesssim 10^{-3}$ in modern storage rings; the framework fixes the exponent $p=4$ from gradient coupling (SI Section~\ref{si:drag}).
...
\begin{equation}
  \tau_{\rm eff}^2(x,a) \;=\; \tau_{\rm bg}^2(x) \; +\; \lambda_U\,T_U(a)^2,\qquad T_U(a)=\frac{\hbar_{\rm eff} a}{2\pi c_s k_B}.
\end{equation}
