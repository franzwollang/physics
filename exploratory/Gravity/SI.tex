...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Clock Rate vs Ruler Scale (Resolution of Paradox):} A shrinking ruler ($L_*\propto 1/\tau$) implies a slowing clock. Since $c_s$ is locally constant, light travels 1 ruler-length per tick. If the ruler shrinks relative to the coordinate background, a fixed coordinate interval represents \emph{more} ruler-lengths, requiring \emph{more} ticks to traverse. Thus, local processes appear slower to a distant observer, resolving the apparent contradiction between "smaller size" (higher natural frequency) and "slower time" (redshift).
...
Proper acceleration shifts the comoving window's effective UV edge: a non\,inertial (Rindler) trajectory samples the vacuum fluctuations of the massless phase sector with a different mode decomposition than an inertial trajectory. Since the phase sector in this framework is governed (in the continuum window) by a hyperbolic operator with dispersion $\omega^2=c_s^2 k^2$, the standard kinematic argument applies: the restriction of the vacuum correlator to a uniformly accelerated worldline has a thermal (KMS) form. It is convenient to summarize the resulting additional excitation of internal degrees of freedom as an Unruh\,like contribution
\begin{equation}
  T_U(a)=\frac{\hbar_{\rm eff}\, a}{2\pi c_s k_B}\,,
\end{equation}
(Unruh, 1976; see also Birrell\,\&\,Davies) entering the effective noise scale used for drag. In the present framework, one may equivalently interpret this as: forced acceleration makes previously ``too fast'' (UV) fluctuations operationally resolvable to the soliton's internal mode, i.e. it shifts the effective comoving UV edge of the window. Detailed consequences for non\,geodesic motion (the drag law, the thermodynamic interpretation, and equivalence considerations) are developed in Section~\ref{si:drag}; they are not needed for the window justification itself.
...
\paragraph{Intuition: ``UV access'' $\Rightarrow$ a backward thermodynamic pressure (Tolman gradient).}
The Unruh/KMS statement above can be re-expressed in the language used throughout this paper: the internal equilibrium of a massive soliton is controlled by the \emph{effective bath intensity} it samples. Forced acceleration makes the effective bath ``hotter'' (more noisy) by shifting the operational window, and crucially, it does so \emph{anisotropically} across a finite-size object.

For a uniformly accelerated frame (Rindler chart) there is a natural length scale
\begin{equation}
  \lambda_{\rm Rindler} \;\sim\; \frac{c_s^2}{a},
\end{equation}
the distance to the horizon behind the accelerated observer. The local temperature in a static gravitational field (and likewise in Rindler) obeys the Tolman--Ehrenfest relation $T\sqrt{-g_{00}}=\text{const}$. In Rindler form, taking $\xi$ as the coordinate along the acceleration axis with $\xi=0$ at the soliton center and $+\xi$ in the acceleration direction, one has (to leading order for $|\xi|\ll \lambda_{\rm Rindler}$)
\begin{equation}
  T(\xi) \;=\; \frac{T_U}{1 + a\xi/c_s^2} \;\simeq\; T_U\left(1 - \frac{a\xi}{c_s^2}\right),\qquad
  \frac{\partial T}{\partial \xi}\Big|_{\xi=0} \;=\; -\,\frac{a}{c_s^2}\,T_U\,,
\end{equation}
so the ``trailing'' side ($\xi<0$, toward the horizon) is slightly hotter than the ``leading'' side ($\xi>0$). A finite-size soliton of extent $\sigma$ therefore sees a fractional bath gradient
\begin{equation}
  \frac{\Delta T}{T} \;\sim\; \frac{a\,\sigma}{c_s^2}\,.
\end{equation}
Since the soliton's relaxed free energy is a decreasing function of bath intensity (main text, Section~3.2), this spatial gradient in the effective noise proxy $\tau_{\rm eff}(\xi)$ generates a thermodynamic force of the same form as gravity,
\begin{equation}
  \mathbf F_{\rm drag} \;\propto\; +\,\nabla \tau_{\rm eff}\,,
\end{equation}
which points toward \emph{higher} bath intensity --- i.e. toward $\xi<0$, opposite the acceleration direction. This is the drag: forced acceleration creates (in the comoving description) an effective noise gradient across the soliton, and the soliton responds by being ``pulled'' backward into the hotter side.

\paragraph{Energy bookkeeping (sign).} The drag does negative work on the soliton. Under sustained forcing, external work is continually converted into phase-sector excitations and internal dissipation: kinetic energy is \emph{bled off} into the bath. This is consistent with the Kubo/FDT calculation below, which computes the dissipated power. On geodesic motion the proper acceleration vanishes ($a=0$), so there is no Unruh/Tolman contribution and this dissipative channel shuts off.

\medskip
\noindent Let $\mathbf a_0$ denote the apparent acceleration in the lab frame (e.g., from a background gradient), and $\hat v$ the unit velocity direction. The proper acceleration experienced in the comoving frame is
\begin{equation}
...
\paragraph{(i) Linear response (Kubo) with Unruh bath.} Let a dominant internal coordinate $q$ couple to a bath operator $B$ sourced by proper acceleration ($T_U=\hbar_{\rm eff} a/(2\pi c_s k_B)$; Unruh 1976; Birrell\,\&\,Davies): $H_{\rm int}=-q\,B$. The dissipated power in the comoving frame is
\begin{equation}
...
\medskip
\noindent\textbf{Phenomenological distinction.} This framework predicts that a self-gravitating, weakly dissipative halo component relaxes toward an isothermal envelope with $v_{\rm flat}^2\simeq2\sigma^2$ (derived in Section~\ref{si:isothermal}) and a tiny $w\approx-1$ drift from sliding windows. In the unified soliton sector this halo component is identified with the $l=0$ (breathing-mode) solitons discussed in the Dark Matter manuscript: they are collisionless/low-dissipation in local units (no phase friction) and therefore remain pressure-supported rather than collapsing into a disk.
...
\paragraph{Flat rotation curves (halo interpretation).} The isothermal profile emerges as the Jeans equilibrium of a self-gravitating, weakly dissipative soliton gas coupled to the phase bath. In the present corpus, the required long-lived halo population is provided by the $l=0$ (breathing-mode) solitons (Dark Matter manuscript). Baryonic ($l=1$) solitons experience phase friction and can dissipate into disks; the $l=0$ sector remains approximately collisionless and forms diffuse halos whose equilibrium envelope yields flat rotation curves.
